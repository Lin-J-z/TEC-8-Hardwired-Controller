module basic_task_int(
    ...
    input INT,                 // 【新增】外部中断请求信号（高电平有效，接 PIN_17）
...
// ---------- 内部状态信号（非外部引脚） ----------
    ...
    always @(negedge T3 or negedge CLR) begin
        if (!CLR) begin
            ST0 <= 1'b0;
            my_pc_tracker <= 8'h00; // 复位时，追踪器与 PC 同步清零归位
        end else begin
            if (SST0)
                ST0 <= ~ST0;
                
            // 只要PCINC，且没有发生中断，追踪器同步自增
            if (PCINC && !INT)
                ...
                MODE_AUTO: begin
                    if (ST0) begin          // ST0=1：第二个节拍
                        ...
                        if (INT) begin
                            ...
                            //计数器值通过 ALU 进寄存器 R3
                            M = 1'b1; {S3, S2, S1, S0} = 4'b1010; // ALU 调至直通
                            ABUS = 1'b1;    // 打开 ALU 输出门
                            SELCTL = 1'b1;  {SEL3, SEL2, SEL1, SEL0} = 4'b1111; // 选择 R3
                            DRW = 1'b1;     // 写使能
                        end
                        // =======================================================================
                        else begin
                            ...
                            if (W2) begin
                                case ({IR7, IR6, IR5, IR4})
                                    ...
                                    // ==================== 【新增：中断返回】 ====================
                                    OPCODE_INT: begin
                                        M = 1'b1; {S3, S2, S1, S0} = 4'b1010; // ALU 直通模式
                                        SELCTL = 1'b1; {SEL3, SEL2, SEL1, SEL0} = 4'b1111; // 选择 R3
                                        ABUS = 1'b1;                          // R3 里的断点值上总线
                                        LPC = 1'b1;
                                    end
                                    // =======================================================================
